...
export class StoryParser {
  ...
  static extractAnimations(text) {
    // Match {Animation:ActionName} or {ActionName} but NOT other namespaced tags
    const matches = text.matchAll(/\{Animation:([^}]+)\}/g);
    const animations = Array.from(matches).map((m) => m[1]);

    // Also match bare capitalized tags like {WaveHand}, {EyeOpen}
