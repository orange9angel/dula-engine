...
export class FacialAnimationSystem {
  ...
  _updateBlink(delta) {
    ...
    // 自动触发
    if (!this.blink.active && time >= this.blink.nextBlinkTime) {
      this.triggerBlink();
    }
  ...
  _scheduleNextBlink() {
    ...
    this.blink.nextBlinkTime = (this._lastTime || 0) + this._blinkIntervalMin + Math.random() * range;
  }
  ...
  _applyMixedFacial() {
    ...
    const mixedJawOpen = V.active
      ? (V.jawOpen * V.weight * tensionFactor + E.lipOffset * w) * (1 - E.eyelidClosed * 0.1)
      : (E.lipOffset * w);
    const mixedLipWidth = (V.active ? V.lipWidth : 1) + E.lipWidth * w * 0.5;
