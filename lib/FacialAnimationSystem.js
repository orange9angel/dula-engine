...
export class FacialAnimationSystem {
  constructor(character) {
    ...
    this.emotion = {         // 表情驱动
      ...
      lipOffset: 0,         // 嘴部垂直偏移
      lipWidth: 0,          // 嘴部宽度变化
      jawOpen: 0,           // 表情驱动的额外张嘴（仅在说话时叠加）
      // 眉毛
      browLeft: 0,          // 左眉毛高度偏移 (-1 下压 ~ +1 上提)
      browRight: 0,
      browInner: 0,         // 眉心挤压
      // 眼睛
      eyelidClosed: 0,      // 兼容旧调用：0=全开, 1=全闭
      eyelidClosedLeft: 0,
      eyelidClosedRight: 0,
      eyeSquint: 0,         // 兼容旧调用
      eyeSquintLeft: 0,
      ...
      pupilDilate: 0,
      // 眼皮可见性
      eyelidVisibleLeft: false,
      eyelidVisibleRight: false,
      // 整体
      faceTilt: 0,          // 面部倾斜
      headNod: 0,           // 点头/仰头
    };
   ...
   * @param {string} shape - viseme ID
   * @param {number} weight - 0~1 权重
   * @param {object} [options] - 可选覆盖，如 { jawOpen: 0.6 }
   */
  setViseme(shape, weight = 1, options = {}) {
    ...
    this.viseme.jawOpen = options.jawOpen !== undefined ? options.jawOpen : table.jawOpen;
  ...
  setEmotion(name, params = {}) {
    ...
    this.emotion.jawOpen = params.jawOpen ?? 0;
    ...
    const sharedEyelidClosed = params.eyelidClosed ?? 0;
    const sharedEyeSquint = params.eyeSquint ?? 0;
    this.emotion.eyelidClosed = sharedEyelidClosed;
    this.emotion.eyelidClosedLeft = params.eyelidClosedLeft ?? sharedEyelidClosed;
    this.emotion.eyelidClosedRight = params.eyelidClosedRight ?? sharedEyelidClosed;
    this.emotion.eyeSquint = sharedEyeSquint;
    this.emotion.eyeSquintLeft = params.eyeSquintLeft ?? sharedEyeSquint;
    this.emotion.eyeSquintRight = params.eyeSquintRight ?? sharedEyeSquint;
    this.emotion.pupilDilate = params.pupilDilate ?? 0;
    this.emotion.eyelidVisibleLeft = params.eyelidVisibleLeft ?? false;
    this.emotion.eyelidVisibleRight = params.eyelidVisibleRight ?? false;
  ...
  clearEmotion() {
    ...
    this.emotion.eyelidVisibleLeft = false;
    this.emotion.eyelidVisibleRight = false;
  }
  ...
  _applyMixedFacial() {
    ...
    const emotionJawOpen = V.active ? E.jawOpen * w * 0.5 : 0;
    const mixedJawOpen = V.active
      ? (V.jawOpen * V.weight * tensionFactor + emotionLipOffset + emotionJawOpen) * (1 - E.eyelidClosed * 0.1)
      : 0;
    const mixedLipWidth = (V.active ? V.lipWidth : 1) + E.lipWidth * w * 0.2;
    const mixedLipRound = V.lipRound * (V.weight || 0) + E.tension * w * 0.3;

    // 应用嘴型（基于角色原始嘴型缩放，避免闭嘴时被拉成大口）
    const baseSX = char.mouthBaseScaleX || 1;
    const baseSY = char.mouthBaseScaleY || 1;
    const baseSZ = char.mouthBaseScaleZ || 1;
    if (char.mouth) {
      char.mouth.scale.set(
        baseSX * mixedLipWidth,
        baseSY * (1 + mixedJawOpen * 0.6),
        baseSZ
      );
    ...
    if (B.active) {
      ...
      // 表情半闭 + 眯眼
      leftLid = Math.min(1, E.eyelidClosedLeft * w + E.eyeSquintLeft * w * 0.5);
      rightLid = Math.min(1, E.eyelidClosedRight * w + E.eyeSquintRight * w * 0.5);
    }

    const showLeftEyelid = E.eyelidVisibleLeft || B.active;
    const showRightEyelid = E.eyelidVisibleRight || B.active;
    if (char.leftEyelid) {
      const baseScaleY = char.leftEyelid.userData.dulaBaseScaleY
        ?? char.leftEyelidBaseScaleY
        ?? char.leftEyelid.scale.y;
      char.leftEyelid.userData.dulaBaseScaleY = baseScaleY;
      char.leftEyelid.visible = showLeftEyelid;
      char.leftEyelid.position.y = (char.leftEyelidBaseY || 0) - leftLid * 0.04;
      char.leftEyelid.scale.y = baseScaleY * Math.max(0.05, 1 - leftLid * 0.95);
    }
    if (char.rightEyelid) {
      const baseScaleY = char.rightEyelid.userData.dulaBaseScaleY
        ?? char.rightEyelidBaseScaleY
        ?? char.rightEyelid.scale.y;
      char.rightEyelid.userData.dulaBaseScaleY = baseScaleY;
      char.rightEyelid.visible = showRightEyelid;
      char.rightEyelid.position.y = (char.rightEyelidBaseY || 0) - rightLid * 0.04;
      char.rightEyelid.scale.y = baseScaleY * Math.max(0.05, 1 - rightLid * 0.95);
    }

    // ── 瞳孔/眼球追踪 ──
    const leftPupil = char.leftPupil || char.eyeLeft;
    const rightPupil = char.rightPupil || char.eyeRight;
    const pupilScale = Math.max(0.35, 1 + E.pupilDilate * w * 0.3);
    for (const pupil of [leftPupil, rightPupil]) {
      if (!pupil?.scale) continue;
      if (!pupil.userData.dulaBaseScale) pupil.userData.dulaBaseScale = pupil.scale.clone();
      pupil.scale.copy(pupil.userData.dulaBaseScale).multiplyScalar(pupilScale);
    }
