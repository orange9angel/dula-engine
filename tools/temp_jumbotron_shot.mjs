...
server.listen(PORT, async () => {
  ...
  // Seek to t=5s and set camera to look up at jumbotron
  const dataUrl = await page.evaluate(async () => {
    const t = 5.0;
    const PREWARM_TIME = Math.max(0, t - 0.5);
    window.storyboard.update(PREWARM_TIME);
    window.storyboard.update(t);
    const cam = window.storyboard.camera;
  ...
  const filename = path.join(CHECK_DIR, 'jumbotron_shot.jpg');
  fs.writeFileSync(filename, buffer);
  console.log(`Jumbotron shot saved: ${filename}`);
